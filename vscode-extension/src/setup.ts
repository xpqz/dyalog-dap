...
export function starterLaunchConfiguration(adapterPath: string): LaunchConfiguration {
  return {
...
export function validateRideAddress(value: string): SetupValidation {
...
export function validateAdapterPathCandidate(
  ...
  if (text === "") {
    return {
      ok: false,
      message:
        "Adapter path is empty. Build dap-adapter (go build ./cmd/dap-adapter) or set DYALOG_DAP_ADAPTER_PATH."
    };
  }
  ...
  if (!fileExists(text)) {
    return {
      ok: false,
      message:
        `Adapter not found at ${text}. Build dap-adapter (go build ./cmd/dap-adapter) or set DYALOG_DAP_ADAPTER_PATH.`
    };
  }
...
export function ensureLaunchConfigText(
  currentText: string,
  starter: LaunchConfiguration
): { changed: boolean; text: string; error?: string } {
  ...
  if (trimmed !== "") {
    ...
    if (!parsed.ok) {
      return {
        ...
        text: currentText,
        error:
          "launch.json contains invalid JSON syntax. Fix launch.json and re-run 'Dyalog DAP: Setup Launch Configuration'."
      };
    }
    doc = {
  ...
  const alreadyPresent = doc.configurations.some((cfg) => {
    ...
    const typeValue = (cfg as Record<string, unknown>).type;
  ...
  doc.configurations.push(starter as Record<string, unknown>);
  return {
    changed: true,
    text: JSON.stringify(doc, null, 2) + "\n"
  };
...
function parseLaunchDocument(text: string):
  | { ok: true; version: string; configurations: Record<string, unknown>[] }
  | { ok: false } {
  try {
    ...
    const parsedConfigs = Array.isArray(parsed.configurations)
      ? (parsed.configurations as Record<string, unknown>[])
      : [];
    return {
      ok: true,
